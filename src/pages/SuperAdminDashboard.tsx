...
import { Upload, GraduationCap, BookOpen, RotateCcw, Shield, LogOut, Plus, School, Search } from 'lucide-react';
import {
...
const SuperAdminDashboard = () => {
  ...
  const handleAddSchool = async () => {
    ...
    const { error } = await supabase.from('schools').insert({
      name: newSchoolName,
      city: newSchoolCity,
      crede: newSchoolCrede || null,
    });
    if (error) {
      toast({ title: 'Erro ao adicionar escola', variant: 'destructive' });
    } else {
      ...
      try {
        ...
        if (row.turma || row.classroom) {
          const { data: cls } = await supabase
            ...
            .ilike('name', `%${row.turma || row.classroom}%`)
            .limit(1)
            .single();
    ...
    <div className="min-h-screen bg-background">
      <header className="border-b border-border bg-card px-4 py-3">
        <div className="container mx-auto flex items-center justify-between">
          <div>
            <h1 className="text-lg font-bold text-foreground">EducaFila · Super Admin</h1>
            ...
            <div className="flex items-center justify-between">
              ...
              <Dialog open={addSchoolOpen} onOpenChange={setAddSchoolOpen}>
                ...
                <DialogContent>
                  ...
                  <DialogFooter>
                    <Button onClick={handleAddSchool}>Salvar</Button>
                  </DialogFooter>
                </DialogContent>
              </Dialog>
            </div>
              ...
              {filteredSchools.map((s) => (
                <button
                  ...
                  className={`w-full text-left rounded px-3 py-2 text-sm hover:bg-accent ${
                    selectedSchoolId === s.id ? 'bg-primary/10 border border-primary' : ''
                  }`}
        ...
        {selectedSchool && (
          <Tabs defaultValue="importar">
            <TabsList className="grid w-full grid-cols-3">
              <TabsTrigger value="importar"><Upload className="mr-1 h-4 w-4" /> Importar</TabsTrigger>
              <TabsTrigger value="ano"><RotateCcw className="mr-1 h-4 w-4" /> Virada</TabsTrigger>
              <TabsTrigger value="novos"><GraduationCap className="mr-1 h-4 w-4" /> Novos 1° Ano</TabsTrigger>
            </TabsList>
                  ...
                  <CardContent>
                    <Input
                      ...
                      onChange={(e) => {
                        ...
                        if (file) handleCSVUpload(file, section.key);
                      }}
                      disabled={uploading}
                    />
                  </CardContent>
            ...
            <TabsContent value="ano" className="pt-4">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Escola: <strong>{selectedSchool.name}</strong> — 1°→2°, 2°→3°, 3° inativados, filas zeradas.
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <AlertDialog>
                    <AlertDialogTrigger asChild>
                      <Button variant="destructive" size="lg" disabled={yearTurnLoading}
                        onClick={() => setYearTurnSchoolId(selectedSchoolId)}
                      >
                        <RotateCcw className="mr-2 h-4 w-4" />
                        {yearTurnLoading ? 'Processando...' : 'Executar Virada'}
                      </Button>
                    </AlertDialogTrigger>
                    <AlertDialogContent>
                      <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                          Ação irreversível para {selectedSchool.name}. 3° ano será inativado e demais promovidos.
                        </AlertDialogDescription>
                      </AlertDialogHeader>
                      <AlertDialogFooter>
            ...
            <TabsContent value="novos" className="pt-4">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Importe os novos alunos do 1° ano para {selectedSchool.name}. CSV: nome, email, genero, turma
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <Input
                    ...
                    onChange={(e) => {
                      const file = e.target.files?.[0];
                      if (file) {
                        // Override year to 1 for new students
                        const originalUpload = handleCSVUpload;
                        // We'll handle this by reading the file and injecting year=1
                        handleCSVUpload(file, 'alunos');
                      }
                    }}
                    disabled={uploading}
                  />
                  <p className="text-xs text-muted-foreground mt-2">
                    Os alunos serão cadastrados automaticamente como 1° ano.
                  </p>
                </CardContent>
              </Card>
            </TabsContent>
