...
const TeacherDashboard = () => {
  ...
  // Fetch classrooms for active school
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'queue_entries', filter: `classroom_id=eq.${selectedClassroom}` }, fetchQueue)
      .subscribe();
  ...
  const applyPenalty = async (userId: string, userName: string) => {
    if (!activeSchoolId) return;
    ...
    await supabase.from('penalties').insert({
      ...
      reason: 'Penalidade aplicada pelo professor',
      infraction_number: infractions,
      penalty_percent: Math.min(percent, 100),
      applied_by: user?.id,
    });
  ...
  const movePosition = async (entryId: string, direction: 'up' | 'down') => {
    const idx = queue.findIndex((e: any) => e.id === entryId);
    if (idx < 0) return;
  ...
  const getBathroomSeconds = (entry: any): number | null => {
    if (entry.status !== 'in_bathroom') return null;
    // Use updated_at as proxy for when status changed to in_bathroom
  ...
  const exceededStudents = inBathroom.filter((e: any) => {
    const sec = getBathroomSeconds(e);
    return sec !== null && sec > 360;
  });

  const avgTime = useMemo(() => {
    const times = inBathroom.map((e: any) => getBathroomSeconds(e)).filter((t): t is number => t !== null);
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <Card>
                <CardContent className="py-3 text-center">
                  ...
                  <p className="text-2xl font-bold text-foreground">{avgTime > 0 ? formatTime(avgTime) : '--'}</p>
                  <p className="text-xs text-muted-foreground">Tempo médio</p>
                </CardContent>
              </Card>
            </div>

            {/* Alerts for exceeded time */}
            {exceededStudents.map((entry: any) => (
              <div key={entry.id} className="flex items-center gap-2 rounded-lg border border-destructive bg-destructive/10 p-3">
                <AlertTriangle className="h-5 w-5 text-destructive" />
                <span className="text-sm font-medium text-destructive">
                  {(entry.profiles as any)?.full_name} — {formatTime(getBathroomSeconds(entry)!)} no banheiro (excedeu 6 min)
                </span>
              </div>
            ))}
                        ...
                        >
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-muted-foreground">
                                {entry.status === 'in_bathroom' ? (
                                  <span className={exceeded ? 'text-destructive font-semibold' : 'text-warning'}>
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            <Button
                              ...
                              size="icon"
                              className="h-8 w-8"
                              onClick={() => applyPenalty(entry.user_id, (entry.profiles as any)?.full_name)}
