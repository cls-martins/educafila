...
const StudentDashboard = () => {
  ...
  const handleJoinQueue = async () => {
    ...
    setLoading(true);
    await enterQueue(user.id, classroomId, activeSchoolId);
    await fetchQueue();
    setLoading(false);
  };
  ...
  const handleGoToBathroom = async () => {
  ...
  const handleReturnFromBathroom = async () => {
    if (!myEntry || !user || !classroomId || !activeSchoolId) return;
    setLoading(true);
  ...
  const handleRequestSwap = async () => {
    ...
    setLoading(true);
    await requestSwap(user.id, swapTargetId, classroomId, activeSchoolId);
    toast({ title: 'Solicitação enviada', description: 'Aguardando confirmação do outro aluno.' });
    ...
    <div className="min-h-screen bg-background">
      ...
      <header className="border-b border-border bg-card px-4 py-3">
        <div className="container mx-auto flex items-center justify-between">
          <div>
            <h1 className="text-lg font-bold text-foreground">EducaFila</h1>
      ...
      <main className="container mx-auto px-4 py-6 space-y-6">
        ...
        {isInBathroom && (
          <Card className={`border-2 ${timerWarning ? 'border-destructive' : 'border-primary'}`}>
            <CardContent className="flex flex-col items-center py-8">
              ...
              <p className={`text-4xl font-mono font-bold ${timerWarning ? 'text-destructive' : 'text-foreground'}`}>
                {formattedTime}
              </p>
              ...
              <Button
                ...
                size="lg"
                className="mt-4"
                onClick={handleReturnFromBathroom}
                disabled={loading}
              >
                Registrar Volta
              </Button>
            ...
            <CardContent className="space-y-4">
              {!myEntry ? (
                <Button onClick={handleJoinQueue} disabled={loading} className="w-full" size="lg">
                  <LogIn className="mr-2 h-4 w-4" /> Entrar na Fila
                </Button>
              ) : (
                <div className="space-y-3">
                  <div className="flex items-center justify-between rounded-lg bg-secondary p-4">
                    ...
                    <Badge variant={isFirstInQueue ? 'default' : 'secondary'}>
                      {isFirstInQueue ? 'Sua vez!' : 'Aguardando'}
                    </Badge>
                  </div>

                  {isFirstInQueue && (
                    <Button onClick={handleGoToBathroom} disabled={loading} variant="hero" className="w-full" size="lg">
                      🚻 Ir ao Banheiro
                    </Button>
                  )}

                  <div className="flex gap-2">
                    <Button onClick={handleLeaveQueue} disabled={loading} variant="destructive" className="flex-1">
                      <LogOut className="mr-1 h-4 w-4" /> Sair da Fila
                    </Button>
                    <Button
                      variant="outline"
                      className="flex-1"
                      disabled={loading || queue.length < 2 || !myEntry}
                      onClick={() => setSwapDialogOpen(true)}
                    >
                      <ArrowLeftRight className="mr-1 h-4 w-4" /> Trocar de Canto
                    </Button>
                  </div>
                </div>
              )}
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base">Fila Atual ({queue.length})</CardTitle>
          </CardHeader>
          <CardContent>
            {queue.length === 0 ? (
              <p className="text-center text-sm text-muted-foreground py-4">Fila vazia</p>
            ) : (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm font-medium text-foreground">
                        {(entry.profiles as any)?.full_name || 'Aluno'}
                      </span>
                    </div>
                    <Badge variant={
                      entry.status === 'in_bathroom' ? 'destructive' :
                      entry.status === 'returned' ? 'default' : 'secondary'
                    }>
                      {entry.status === 'in_bathroom' ? '🚻 No banheiro' :
                       entry.status === 'returned' ? 'Voltou' : 'Aguardando'}
                    </Badge>
                  </div>
                ))}
              </div>
      ...
      <AlertDialog open={swapDialogOpen} onOpenChange={setSwapDialogOpen}>
        <AlertDialogContent>
              ...
              .map((entry) => (
                ...
                >
                  <span className="text-sm font-medium">
                    {entry.position}° — {(entry.profiles as any)?.full_name || 'Aluno'}
                  </span>
                </button>
              ))}
          </div>
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={handleRequestSwap} disabled={!swapTargetId || loading}>
              Solicitar Troca
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      ...
      <AlertDialog open={incomingSwapDialogOpen} onOpenChange={setIncomingSwapDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Um colega deseja trocar de posição com você na fila. Deseja aceitar?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
